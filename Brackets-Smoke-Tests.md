...
1. Back in Brackets, on line 26, put the cursor after "A new" and type ` and totally <em>AWESOME</em> local`; let the `<em>` tag autocomplete (you can arrow or click select after the `</em>` to type `local`). Verify that the changes appear in the browser as you type (except that nothing updates while you type `<em`), and verify that AWESOME is italicized (and nothing else is).
