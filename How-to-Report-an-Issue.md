...
4. If you can't reproduce the issue, move extensions back to the `user` directory one at a time (restarting Brackets each time) until you find the cause. If you find an extension bug, please file an issue in that extension's repo so it can be addressed by the extension author.
